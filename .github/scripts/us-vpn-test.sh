...
STAGE="install-runtime"; publish_state starting "$STAGE" "Installing runtime"
sudo apt-get update
...
STAGE="select-native-tcp443-us"; publish_state starting "$STAGE" "Selecting native US TCP 443 VPN Gate configs"
printf 'vpn\nvpn\n' >/tmp/vpn-auth
python3 - <<'PY'
...
for attempt in range(1,6):
    try:
        raw=urllib.request.urlopen('https://www.vpngate.net/api/iphone/',timeout=30).read().decode('utf-8','replace'); break
    except Exception as exc:
        last=exc; print(f'VPN Gate API attempt {attempt}/5 failed: {exc}', flush=True)
        if attempt < 5: time.sleep(min(3*attempt,10))
if raw is None: raise SystemExit(f'VPN Gate API unavailable after retries: {last}')
lines=[x for x in raw.splitlines() if x.strip() and not x.startswith('*')]
i=next(i for i,x in enumerate(lines) if x.startswith('#HostName,')); lines=lines[i:]; lines[0]=lines[0].lstrip('#')
rows=list(csv.DictReader(io.StringIO('\n'.join(lines)))); out=[]
for r in rows:
    if (r.get('CountryShort') or '').upper()!='US': continue
...
if not out: raise SystemExit('No native United States TCP443 config')
json.dump(out[:30],open('/tmp/us.json','w',encoding='utf-8'))
print('native tcp443 US:',len(out))
PY

STAGE="connect-us-exit"; publish_state starting "$STAGE" "Connecting native US TCP 443 exit and verifying US"
ORIGINAL_IP="$(curl -4fsS --max-time 15 https://api.ipify.org || true)"; FOUND=0
for IDX in $(seq 0 29); do
  ITEM="$(jq -r ".[${IDX}] // empty" /tmp/us.json)"; [ -n "$ITEM" ] || break
  IP="$(echo "$ITEM"|jq -r '.ip')"; echo "$ITEM"|jq -r '.config' >/tmp/us.ovpn
  if grep -qE '^auth-user-pass([[:space:]]|$)' /tmp/us.ovpn; then sed -i -E 's#^auth-user-pass.*#auth-user-pass /tmp/vpn-auth#' /tmp/us.ovpn; else echo 'auth-user-pass /tmp/vpn-auth' >>/tmp/us.ovpn; fi
  printf '\nauth-nocache\nconnect-retry-max 1\nconnect-timeout 10\n' >>/tmp/us.ovpn
  sudo pkill openvpn >/dev/null 2>&1 || true; sleep 2
  sudo openvpn --config /tmp/us.ovpn --daemon qifu-us --writepid /tmp/openvpn.pid --log /tmp/openvpn.log || true
  ...
  if [ -n "$EXIT_IP" ] && [ "$EXIT_IP" != "$ORIGINAL_IP" ] && [ "$COUNTRY" = US ]; then printf '%s' "$EXIT_IP" >/tmp/us-exit-ip; printf '%s' "$IP" >/tmp/us-vpngate-ip; FOUND=1; break; fi
  sudo pkill openvpn >/dev/null 2>&1 || true
done
[ "$FOUND" -eq 1 ] || false

STAGE="fix-dns"; publish_state starting "$STAGE" "US exit verified; switching runner to public DNS"
...
STAGE="start-public-relay"; publish_state starting "$STAGE" "Starting VLESS WebSocket and Cloudflare Quick Tunnel"
...
STAGE="wait-public-dns"; publish_state starting "$STAGE" "Waiting for US trycloudflare hostname to resolve"
DNS_OK=0
...
URL_FULL="${URL}/sub.yaml" EXIT_IP="$PUBLIC_IP" VPN_GATE_IP="$(cat /tmp/us-vpngate-ip)" STARTED="$STARTED" EXPIRES="$EXPIRES" python3 - <<'PY'
import json,os
x={'ready':True,'status':'ready','stage':'ready','subscription_url':os.environ['URL_FULL'],'stable_subscription_path':'subscriptions/us-test.yaml','verified_country':'US','verified_exit_ip':os.environ['EXIT_IP'],'vpn_gate_server_ip':os.environ['VPN_GATE_IP'],'started_at':os.environ['STARTED'],'approx_expires_at':os.environ['EXPIRES'],'run_id':os.environ.get('GITHUB_RUN_ID',''),'transport':'VLESS+WebSocket+TLS'}
open('/tmp/us-state.json','w',encoding='utf-8').write(json.dumps(x,ensure_ascii=False,indent=2))
PY
